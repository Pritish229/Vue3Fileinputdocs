<template>
  <div>
    ...
    >
      ...
      <div v-if="!selectedFile" :class="{ 'placeholder-error': inValid, placeholder: !inValid }">
        <slot name="icon">
          <i class="fas fa-upload upload-icon fs-4"></i>
        </slot>
        <p class="upload-text" :class="{ 'text-danger': inValid, placeholder: !inValid }">{{ placeholder }}</p>
      </div>

      <!-- Preview -->
      <div v-else class="preview">
        <template v-if="preview.startsWith('data:image/')">
          <img :src="preview" :alt="fileTypeLabel" class="preview-image" />
        </template>
        <template v-else>
          <i :class="preview" class="fs-1"></i>
        </template>
      </div>
...
const props = defineProps({
  ...
  fileType: {
    type: Array,
    default: () => ['image/png', 'image/jpeg', 'image/jpg', 'application/pdf', 'text/plain', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'],
  },
  placeholder: {
    type: String,
...
const processFile = (file) => {
  ...
  if (file.size > props.maxSize) {
    showError(`File is too large. Maximum size is ${(props.maxSize / (1024 * 1024)).toFixed(2)}MB.`)
    return
  }
  errorMessage.value = ''
...
const generatePreview = (file) => {
  const fileIcons = {
    'application/pdf': 'fas fa-file-pdf text-danger fs-1',
  ...
  if (file.type.startsWith('image/')) {
    ...
    preview.value = fileIcons[file.type] || 'fas fa-file text-muted'
  }
}
...
.error-border {
  border: 1.9px dashed #fd625e !important;
}
